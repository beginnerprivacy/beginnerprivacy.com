...
description: 轻松切换到 Linux 的分步指南，从选择发行版到完成安装。
time: 1小时20分钟
date: 2025-03-01
---
### 什么是 Linux？
Linux 是一个自由、开源的操作系统内核，由芬兰学生 Linus Torvalds 于 1991 年创建。由于对昂贵的商业 Unix 系统以及 MINIX 的局限性感到不满，Torvalds 在自己的个人电脑上把开发一个类 Unix 的内核作为一个业余项目开始编写。

<br>

从技术上来说，Linux 只是内核（kernel）——即负责管理硬件（CPU、内存、存储和设备）并运行程序的核心软件。一个完整的操作系统还需要工具、库以及用户界面。GNU 项目（由 Richard Stallman 于 1983 年发起）已经创建了其中的大部分组件，但当时缺少一个内核。当 Torvalds 以自由许可证发布他的内核后，它与 GNU 的工具结合，形成了一个完整的操作系统，通常被称为 Linux（更准确的名称是 GNU/Linux）。

<br>

如今，"Linux"通常指围绕这个内核构建的庞大操作系统家族。世界上存在数百种发行版（"distros"），从适合初学者的 Ubuntu 和 Linux Mint，到面向服务器、嵌入式设备以及各种专用场景的版本。
